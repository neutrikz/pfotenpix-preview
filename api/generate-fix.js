// /api/generate-fix.js – Edit ohne Maske, gpt-image-1, akzeptiert url ODER b64_json, Retries + Soft-Fail
...
// Prompts (Identität stärker sichern)
function buildPrompts(userText) {
  const guardrails = [
    "This is a real photo of the same pet; preserve the exact species and breed-specific morphology.",
    "Keep the same pose, proportions, fur pattern, markings, eye shape and muzzle; no breed change, no cartoon eyes.",
    "Apply only lighting, color grading and stylistic embellishments; do not redraw anatomy; maintain identity.",
    "Ultra-detailed, high-resolution, artifact-free; suitable for fine-art printing."
  ].join(" ");
  ...
  const nat = [
    ...
    "No plastic skin, no HDR, no AI artifacts.",
    guardrails,
    userText ? `If text is provided, integrate it very subtly: "${userText}".` : ""
  ].join(" ");

  const bw = [
    "Fine-art black and white conversion (true grayscale silver-gelatin character).",
    ...
    "No posterization, no color tints.",
    guardrails,
  ...
  const neon = [
    ...
    "Cyan, magenta and orange rim-light strokes following fur contours; smooth neon gradients with gentle halation and glow.",
    "Dark indigo-to-violet vignette background; no anatomy changes; avoid cartoonification.",
    guardrails,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const style of styles) {
      ...
      // ⚠️ keine Maske mehr senden → vermeidet Voll-Neu-Render (Identitäts-Drift)
