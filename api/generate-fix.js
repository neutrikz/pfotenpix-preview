...
function parseMultipart(buffer, boundary) {
  ...
  for (let rawPart of parts){
    ...
    const ctype=headers["content-type"]||""; if (!name) continue;
...
async function fetchOpenAIWithRetry(form, styleName, {retries=3, baseDelayMs=900}={}) {
  ...
  for (let attempt=1; attempt<=retries; attempt++){
    try{
      ...
      const text=await resp.text(); let json; try{ json=JSON.parse(text);}catch{ json=null; }
      if (resp.ok && json?.data?.[0]?.url){ console.log(`🟩 [${styleName}] Erfolg (Versuch ${attempt})${reqId?" – reqId: "+reqId:""}`); return json.data[0].url; }
      const is5xx=resp.status>=500 && resp.status<=599; const isServerErr=json?.error?.type==="server_error";
      const shouldRetry=is5xx||isServerErr;
      console.warn(`🟨 [${styleName}] Fehler (Versuch ${attempt})${reqId?" – reqId: "+reqId:""}`,{ status:resp.status, body:json||text });
      if (!shouldRetry || attempt===retries){ lastError=new Error(json?.error?.message||`OpenAI-Fehler (HTTP ${resp.status}) nach ${attempt} Versuch(en).`); break; }
      const delay=withJitter(baseDelayMs*Math.pow(2,attempt-1)); console.log(`⏳ [${styleName}] Retry in ${delay}ms …`); await sleep(delay);
    }catch(err){
      lastError=err; console.warn(`🟨 [${styleName}] Netzwerk-/Fetch-Error (Versuch ${attempt}):`, String(err));
  ...
  try {
    ...
    for (const style of styles){
      ...
      try{
        ...
        return res.status(502).json({ error:`OpenAI konnte den Stil '${style.name}' nach mehreren Versuchen nicht generieren.` });
      }
    }
