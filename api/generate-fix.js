// /api/generate-fix.js – ohne RemBG, mit Full-Mask (ganze Fläche), Prompts + Retries + offene CORS
...
// --- CORS (permissiv; später gern wieder einschränken)
...
async function fetchOpenAIWithRetry(form, styleName, { retries = 3, baseDelayMs = 900 } = {}) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      const isServerErr = json?.error?.type === "server_error";
      ...
      if (!shouldRetry || attempt === retries) {
        lastError = new Error(json?.error?.message || `OpenAI-Fehler (HTTP ${resp.status}) nach ${attempt} Versuch(en).`);
        break;
      }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 1b) **Full-Mask** (komplett transparent) -> editiere gesamte Fläche
    const fullMaskPng = await sharp({
      create: { width: SIZE, height: SIZE, channels: 4, background: { r: 0, g: 0, b: 0, alpha: 0 } },
    })
      .png()
      .toBuffer();

    // 2) Prompts (Guardrails sichern Erkennbarkeit)
